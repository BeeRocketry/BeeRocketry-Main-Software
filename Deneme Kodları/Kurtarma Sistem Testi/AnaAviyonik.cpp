...
Yer istasyonu seri porttan sürekli olarak gelecek karakterleri  kontrol  edecek  ve  istenen  harfte  veri  gelince  ana
aviyonik sistemin sahip olduğu adres ve frekansa veri paketini yollayacaktır.
...
Cihaz Adresi (High / Low)   --  0x02 0x04
...
#define RFHighAdresi 0x02
#define RFKanal 23U

#define RFGondericiLowAdresi 0x07
...
void loop(){
    if(SeriPort.available()){
        eleman = SeriPort.read();
        DEBUG_PRINTLN();
        
        DEBUG_PRINTLN(F("Seri porta karakter geldi..."));
        delay(1000);
        if(eleman == 'a' || eleman == 'b'){
            DEBUG_PRINTLN(F("Istenen karakter tespit edildi..."));

            messagebuffer[0] = 'y';
            messagebuffer[1] = eleman;
            ...
            DEBUG_PRINTLN(F("Mesaj Gonderildi..."));

            DEBUG_PRINTLN(F("7.5 Saniye Koruma Moduna giriliyor..."));
            delay(7500);
            DEBUG_PRINTLN(F("Koruma Modundan Cikildi..."));
            eleman = 'z';
        }
        else{
            DEBUG_PRINTLN(F("Istenen karakter bulunmadi..."));
        }
    }
    ...
    if(receiveDataPacket(messagebuffer, sizeof(messagebuffer)) == E32_Success){
        if(calculateCRC8(messagebuffer, 2) == messagebuffer[2]){
            if(messagebuffer[0] == 'z' && messagebuffer[1] == 'e'){
                DEBUG_PRINTLN(F("Ana Aviyonik Sistem Baslatildi..."));
                DEBUG_PRINTLN();
                DEBUG_PRINTLN();
            }
        }
